...
**Critical: `"waitFor": "postCreateCommand"`.** By default a devcontainer's `waitFor` is `updateContentCommand`, so JetBrains Gateway attaches the IDE backend *before* `postCreateCommand` runs. The backend then reads an empty (or default) `.idea/`, writes its own `misc.xml` (external storage re-enabled), and clobbers whatever `setup_jetbrains_modules` writes moments later — so siblings silently don't appear. Setting `"waitFor": "postCreateCommand"` in the multi-repo `devcontainer.json` makes Gateway block backend startup until `postCreate` finishes, so IntelliJ reads the *final* module files. This is the fix that makes the setup work with no manual IDE steps; without it the behavior is a timing-dependent race. (Trade-off: first connect waits for the whole `postCreate`, including the base npx preinstall — a one-time per-rebuild cost, not per reconnect.)

**Known caveat (if you skip `waitFor` or hit a reconnect edge case):** on a container whose IDE backend is *already running* when these files are written/corrected, IntelliJ doesn't pick the change up automatically — `modules.xml` needs **Invalidate Caches / Restart**, and the `misc.xml` Maven entry needs an explicit **Reload All Maven Projects** (Maven tool window).
